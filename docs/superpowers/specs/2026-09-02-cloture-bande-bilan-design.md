...
>
> **Statut : validé et livré le 2026-09-02** (PR #279). Les sept décisions du §3 sont
> verrouillées. Le correctif du §1.4 est parti dans la PR #278, mergée avant le chantier.
> `UnitClosureIT` a tourné en CI sur PostgreSQL réel : 4/4, V52 migrée sur base propre. Les sept décisions du §3 sont
> verrouillées. Le correctif du §1.4 est parti dans la PR #278, mergée avant le chantier.
...
- [x] La clôture crée exactement une ligne `unit_closures`, et une seconde tentative renvoie 409.
- [x] Le bilan affiche mortalité, IC, poids de sortie, durée, recettes, coûts détaillés, marge.
- [x] La mortalité figée ignore les ventes.
- [x] Une dépense saisie après la clôture ne modifie pas le bilan.
- [x] Quand un article consommé n'a pas de prix, l'interface le signale.
- [x] Rouvrir supprime le bilan et remet le lot en `ACTIVE`.
- [x] Un membre sans `poultry:read` reçoit 403 ; un FARMER ne peut pas clôturer.
- [x] `./mvnw clean verify` vert, CI verte avant merge.
