 ...
 * lines in full (no partial delivery in V1, D23), decrements PRODUCT stock with one OUT movement
 * per line ({@code reason=SALE}, D21) through {@link StockMovementService}, and marks the order
 * DELIVERED — all atomically. Stock may go negative (D19). Delivery numbers are minted {@code
 * LIV-YYYY-NNN} per farm per year (D24); amounts are HT only (D25). The client balance is NOT
 * touched here (only by payments, B5-4, D26). Cancelling a delivery reverses the stock with
 * compensating IN movements and reopens the order for re-delivery. RBAC is at the controller layer
 * (B5-5).
 */
@Service
...
public class DeliveryService {
  ...
  public Delivery createFromOrder(
    ...
    // D21: decrement stock — one OUT movement (reason SALE) per delivered line.
  ...
  /** DELIVERED → CANCELLED: reverse the stock (compensating IN) and reopen the order. */
  @Transactional
