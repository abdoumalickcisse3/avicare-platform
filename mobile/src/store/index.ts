 ...
 *    `productionUnitsApi` / `authApi` via `injectEndpoints`).
 ...
 *
 * `redux-persist` (see `@/store/persist`) wraps the root reducer with a
 * restricted whitelist (`api` + `selection` only — see that module's header
 * for why). `persistor` is exported for `PersistGate` (root layout) and for
 * the logout/farm-change purge call sites.
 */
import { combineReducers, configureStore } from '@reduxjs/toolkit';
import { FLUSH, PAUSE, PERSIST, PURGE, REGISTER, REHYDRATE } from 'redux-persist';
import { baseApi } from './api/baseApi';
import { selectionReducer } from './slices/selectionSlice';
import { createPersistor, wrapWithPersistence } from './persist';
